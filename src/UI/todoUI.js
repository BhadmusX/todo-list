...
class todoUi {
  ...
  rendermaincontent(projectname, todos) {
    ...
    if (todos.length === 0) {
      ...
      message.textContent =
        "No tasks found for this project. Enjoy your free time!";
      ...
      checklistarray.forEach((check, index) => {
        ...
        checkbox.addEventListener("change", () => {
          ...
          const allChecked = Array.from(
            checklistcont.querySelectorAll('input[type="checkbox"]'),
          ).every((cb) => cb.checked);
          if (allChecked) {
            todotitle.classList.add("disabletitle");
            duedate.classList.add("disableduedate");
    ...
    `;
        ...
        const checklistcont = extendedtodo.querySelector(
          "#edit-checklist-cont",
        );
        ...
        document
          .getElementById("add-new-checklist")
          .addEventListener("click", () => {
            const newitem = document
              .getElementById("new-checklist-input")
              .value.trim();
            if (!newitem) return;
            const div = document.createElement("div");
            const input = document.createElement("input");
        ...
        document.getElementById("save-edit").addEventListener("click", () => {
          ...
          const newchecklist = Array.from(
            checklistcont.querySelectorAll("input"),
          )
  ...
  formtodo(projectName, title, description, duedate, priority, notes) {
    const newtodo = this.app.createtodo(
      projectName,
      title,
      description,
      duedate,
